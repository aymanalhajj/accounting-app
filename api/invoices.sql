...
-- Schema: ACCOUNTING   Date: Tue Oct 15 12:13:08 AST 2024
--
BEGIN
  ORDS.DEFINE_MODULE(
...
BEGIN
    ...
    SALES_PUR_RETURN_INV_DTL_TAPI.DEL_DTL(
        P_INVOICE_ID  => V_INVOICE_ID,
...
BEGIN
    ...
    END;
    L_ITEMS_ARR := L_BODY_OBJ.GET_ARRAY(''items'');
    SALES_PURCHASE_INV_DTL_TAPI.DEL_DTL(
        P_INVOICE_ID  => V_INVOICE_ID,
        P_ID_LIST     =>  L_ITEMS_ARR
    );
    FOR I IN 0..L_ITEMS_ARR.GET_SIZE - 1 LOOP
...
BEGIN
    ...
    L_BODY_OBJ := JSON_OBJECT_T(:body);
    -- step 2: call the procedure to validate inputs
    DATA_VALIDATION.PURCHASE_ORDER_VALIDATE(
        P_DATA => L_BODY_OBJ,
        P_LANG => 2,
        P_RESULT => P_VALID_RESULT
    );
    ...
    END;
    L_ITEMS_ARR := L_BODY_OBJ.GET_ARRAY(''items'');
    SALES_PURCHASE_ORDER_DTL_TAPI.DEL_DTL(
        P_ORDER_ID    => V_ORDER_ID,
        P_ID_LIST     => L_ITEMS_ARR
    );
    FOR I IN 0..L_ITEMS_ARR.GET_SIZE - 1 LOOP
...
BEGIN
    ...
    END;
    L_ITEMS_ARR := L_BODY_OBJ.GET_ARRAY(''items'');
    SALES_RENT_INV_DTL_TAPI.DEL_DTL(
        P_INVOICE_ID  => V_INVOICE_ID,
        P_ID_LIST     =>  L_ITEMS_ARR
    );
    FOR I IN 0..L_ITEMS_ARR.GET_SIZE - 1 LOOP
...
BEGIN
    ...
    END;
    L_ITEMS_ARR := L_BODY_OBJ.GET_ARRAY(''items'');
    SALES_RETURN_INV_DTL_TAPI.DEL_DTL(
        P_INVOICE_ID  => V_INVOICE_ID,
        P_ID_LIST     =>  L_ITEMS_ARR
    ); 
    FOR I IN 0..L_ITEMS_ARR.GET_SIZE - 1 LOOP
