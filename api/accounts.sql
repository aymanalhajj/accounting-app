...
-- Schema: ACCOUNTING   Date: Wed Nov 20 03:04:38 AST 2024
--
BEGIN
  ORDS.ENABLE_SCHEMA(
      p_enabled             => TRUE,
...
END;'
  ...
  ORDS.DEFINE_HANDLER(
      ...
      p_source         => 
'DECLARE
	V_ACCOUNT_ID                   ACC_ACCOUNTS.ACCOUNT_ID%TYPE;
...
BEGIN
   ...
   --   return;
   --END IF;
	V_ACCOUNT_ID := L_BODY_OBJ.GET_STRING(''account_id'');
    IF NVL(V_ACCOUNT_ID,0) = 0 THEN

				ACC_ACCOUNTS_TAPI.ADD_RECORD (
					P_ACCOUNT_NAME_AR  => L_BODY_OBJ.GET_STRING(''account_name_ar''),
					P_ACCOUNT_NAME_EN  => L_BODY_OBJ.GET_STRING(''account_name_en''),
					P_ACCOUNT_TYPE     => L_BODY_OBJ.GET_STRING(''account_type''),
					P_ACCOUNT_ID       => V_ACCOUNT_ID,
					P_ACCOUNT_PARENT   => L_BODY_OBJ.GET_STRING(''account_parent''),
					P_SUB_ACCOUNT      => L_BODY_OBJ.GET_STRING(''sub_account''),
					P_COMPANY_ID       => L_BODY_OBJ.GET_STRING(''company_id''),
					P_ACCOUNT_NATURE   => L_BODY_OBJ.GET_STRING(''account_nature'')
				);
    ELSE
         ACC_ACCOUNTS_TAPI.UPDATE_RECORD (
					P_ACCOUNT_NAME_AR  => L_BODY_OBJ.GET_STRING(''account_name_ar''),
					P_ACCOUNT_NAME_EN  => L_BODY_OBJ.GET_STRING(''account_name_en''),
					P_ACCOUNT_TYPE     => L_BODY_OBJ.GET_STRING(''account_type''),
...
'DECLARE
    ...
    V_RESPONSE      CLOB;
BEGIN
    V_RESPONSE := GET_RECEIPT_VOUCHER(
        P_FIRST       => :p_first,
        P_LAST        => :p_last,
        P_NEXT        => :p_next,
        P_PREV        => :p_prev,
...
BEGIN
    ...
    L_BODY_OBJ := JSON_OBJECT_T(:BODY);
    -- step 2: call the procedure to validate inputs
    DATA_VALIDATION.RECEIPT_VOUCHER_VALIDATE(
       P_DATA => L_BODY_OBJ,
       P_LANG => 2,
       P_RESULT => P_VALID_RESULT
   );
    ...
    BEGIN
        IF NVL(V_ACC_VOUCHER_ID,0) = 0 THEN
            ACC_VOUCHER_TAPI.INSERT_VOUCHER(
                ...
                P_REF_ID            => L_BODY_OBJ.GET_STRING(''ref_id''),
                P_PAYMENT_METHOD    => L_BODY_OBJ.GET_STRING(''payment_method''),
                P_VOUCHER_DATE      => TO_DATE(L_BODY_OBJ.GET_STRING(''voucher_date''),''DD-MM-YYYY''),
                P_VOUCHER_TYPE      => L_BODY_OBJ.GET_STRING(''voucher_type''),
                P_NOTE              => L_BODY_OBJ.GET_STRING(''note''),
                P_COST_CNTR_ID      => L_BODY_OBJ.GET_STRING(''cost_cntr_id''),
                P_CHECK_DATE        => L_BODY_OBJ.GET_STRING(''check_date''),
