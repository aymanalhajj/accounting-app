...
-- Schema: ACCOUNTING   Date: Wed Nov 20 03:03:19 AST 2024
--
BEGIN
  ORDS.ENABLE_SCHEMA(
      p_enabled             => TRUE,
...
END;'
  ...
  ORDS.DEFINE_HANDLER(
      ...
      p_source         => 
'DECLARE
	V_BANK_ACC_ID                   SETUP_BANK_EXT_ACC.BANK_ACC_ID%TYPE;
...
BEGIN
    ...
    DATA_VALIDATION.BANK_ACCOUNT_VALIDATE(
        P_DATA => L_BODY_OBJ,
        P_LANG => 2,
    ...
    IF NVL(V_BANK_ACC_ID,0) = 0 THEN
               ...
               SETUP_BANK_EXT_ACC_TAPI.INSERT_BANK_EXT_ACC (
					...
					P_ACC_NAME_EN      => L_BODY_OBJ.GET_STRING(''acc_name_en''),
					...
					P_ACC_NAME_AR      => L_BODY_OBJ.GET_STRING(''acc_name_ar''),
					P_NOTE             => L_BODY_OBJ.GET_STRING(''note ''),
					P_COMPANY_ID       => L_BODY_OBJ.GET_STRING(''company_id'')
				);
    ELSE
        SETUP_BANK_EXT_ACC_TAPI.UPDATE_BANK_EXT_ACC (
...
'DECLARE
    V_RESPONSE      CLOB;
BEGIN
    SETUP_BANK_EXT_ACC_TAPI.DELETE_BANK_EXT_ACC(
        P_BANK_ACC_ID          => :p_id
    );
END;'
      );
...
END;'
  ...
  ORDS.DEFINE_HANDLER(
      ...
      p_source         => 
'DECLARE
	V_BRANCH_ID                   SETUP_BRANCH.BRANCH_ID%TYPE;
    V_TOKEN                          VARCHAR2(2000);
    L_BODY_OBJ                       JSON_OBJECT_T;
    P_VALID_RESULT                   JSON_OBJECT_T;
BEGIN
    ...
    L_BODY_OBJ := JSON_OBJECT_T(:body);
    -- step 2: call the procedure to validate inputs
    DATA_VALIDATION.BRANCH_VALIDATE(
        P_DATA => L_BODY_OBJ,
        P_LANG => 2,
        P_RESULT => P_VALID_RESULT
    );
    ...
    IF NVL(V_BRANCH_ID,0) = 0 THEN


				SETUP_BRANCH_TAPI.INSERT_BRANCH (
					P_REGION_ID        => L_BODY_OBJ.GET_STRING(''region_id''),
					...
					P_COMERCIAL_REC_NO => L_BODY_OBJ.GET_STRING(''comercial_rec_no''),
					P_MOBILE_NO        => L_BODY_OBJ.GET_STRING(''mobile_no''),
    ...
    ELSE
        SETUP_BRANCH_TAPI.UPDATE_BRANCH (
					P_REGION_ID        => L_BODY_OBJ.GET_STRING(''region_id''),
					P_TAX_NO           => L_BODY_OBJ.GET_STRING(''tax_no''),
					P_COUNTRY_ID       => L_BODY_OBJ.GET_STRING(''country_id''),
					P_SREET            => L_BODY_OBJ.GET_STRING(''sreet''),
...
'DECLARE
    V_RESPONSE      CLOB;
BEGIN
    SETUP_BRANCH_TAPI.DELETE_BRANCH(
        P_BRANCH_ID          => :p_id
    );
END;'
      );
...
END;'
  ...
  ORDS.DEFINE_HANDLER(
      ...
      p_source         => 
'DECLARE
	V_CITY_ID                   SETUP_CITY.CITY_ID%TYPE;
    V_TOKEN                          VARCHAR2(2000);
    L_BODY_OBJ                       JSON_OBJECT_T;
    P_VALID_RESULT                   JSON_OBJECT_T;
BEGIN
    ...
    IF P_VALID_RESULT.GET_STRING(''status'') = ''failed'' THEN
        ...
        return;
    END IF;
	V_CITY_ID := L_BODY_OBJ.GET_STRING(''city_id'');
    IF NVL(V_CITY_ID,0) = 0 THEN

				SETUP_CITY_TAPI.INSERT_CITY (
					P_CITY_ID      => V_CITY_ID,
					P_COUNTRY_ID   => L_BODY_OBJ.GET_STRING(''country_id''),
					P_STATUS       => L_BODY_OBJ.GET_STRING(''status''),
					P_CITY_NAME_AR => L_BODY_OBJ.GET_STRING(''city_name_ar''),
					P_CITY_NAME_EN => L_BODY_OBJ.GET_STRING(''city_name_en''),
					P_COMPANY_ID   => L_BODY_OBJ.GET_STRING(''company_id'')
				);
    ELSE
         SETUP_CITY_TAPI.UPDATE_CITY (
					P_CITY_ID      => L_BODY_OBJ.GET_STRING(''city_id''),
					P_COUNTRY_ID   => L_BODY_OBJ.GET_STRING(''country_id''),
					P_STATUS       => L_BODY_OBJ.GET_STRING(''status''),
					P_CITY_NAME_AR => L_BODY_OBJ.GET_STRING(''city_name_ar''),
...
END;'
  ...
  ORDS.DEFINE_HANDLER(
      ...
      p_source         => 
'DECLARE
	V_COMPANY_ID                   ADMIN_COMPANY.COMPANY_ID%TYPE;
    V_TOKEN                          VARCHAR2(2000);
    L_BODY_OBJ                       JSON_OBJECT_T;
    P_VALID_RESULT                   JSON_OBJECT_T;
BEGIN
    ...
    L_BODY_OBJ := JSON_OBJECT_T(:body);
    -- step 2: call the procedure to validate inputs
   DATA_VALIDATION.COMPANY_VALIDATE(
       P_DATA => L_BODY_OBJ,
       P_LANG => 2,
       P_RESULT => P_VALID_RESULT
   );
   ...
   END IF;
    ...
    IF NVL(V_COMPANY_ID,0) = 0 THEN

				ADMIN_COMPANY_TAPI.INSERT_ADMIN_COMPANY (
    ...
    ELSE
         ADMIN_COMPANY_TAPI.UPDATE_ADMIN_COMPANY (
					  P_COMPANY_ID  => L_BODY_OBJ.GET_STRING(''company_id''),
					  P_NAME_AR     => L_BODY_OBJ.GET_STRING(''name_ar''),
					  P_NAME_EN     => L_BODY_OBJ.GET_STRING(''name_en''),
					  P_MOBILE_NO   => L_BODY_OBJ.GET_STRING(''mobile_no''),
...
'DECLARE
    V_RESPONSE      CLOB;
BEGIN
    ADMIN_COMPANY_TAPI.DELELTE_ADMIN_COMPANY(
        P_COMPANY_ID          => :p_id
    );
END;'
      );
...
END;'
  ...
  ORDS.DEFINE_HANDLER(
      ...
      p_source         => 
'DECLARE
	V_COUNTRY_ID                   SETUP_COUNTRY.COUNTRY_ID%TYPE;
    V_TOKEN                          VARCHAR2(2000);
    L_BODY_OBJ                       JSON_OBJECT_T;
    P_VALID_RESULT                   JSON_OBJECT_T;
BEGIN
    ...
    L_BODY_OBJ := JSON_OBJECT_T(:body);
    -- step 2: call the procedure to validate inputs
    DATA_VALIDATION.COUNTRY_VALIDATE(
        P_DATA => L_BODY_OBJ,
        P_LANG => 2,
        P_RESULT => P_VALID_RESULT
    );
    IF P_VALID_RESULT.GET_STRING(''status'') = ''failed'' THEN
        ...
        return;
    END IF;
	V_COUNTRY_ID := L_BODY_OBJ.GET_STRING(''country_id'');
    IF NVL(V_COUNTRY_ID,0) = 0 THEN
				SETUP_COUNTRY_TAPI.INSERT_COUNTRY (
					P_COUNTRY_ID          => V_COUNTRY_ID,
					P_STATUS              => L_BODY_OBJ.GET_STRING(''status''),
					P_NAME_EN             => L_BODY_OBJ.GET_STRING(''name_en''),
					P_NATIONALITY_NAME_EN => L_BODY_OBJ.GET_STRING(''nationality_name_en''),
					P_NAME_AR             => L_BODY_OBJ.GET_STRING(''name_ar''),
					P_COMPANY_ID          => L_BODY_OBJ.GET_STRING(''company_id''),
					P_NATIONALITY_NAME_AR => L_BODY_OBJ.GET_STRING(''nationality_name_ar'')
				);
    ELSE
        SETUP_COUNTRY_TAPI.UPDATE_COUNTRY (
					P_COUNTRY_ID          => L_BODY_OBJ.GET_STRING(''country_id''),
					P_STATUS              => L_BODY_OBJ.GET_STRING(''status''),
					P_NAME_EN             => L_BODY_OBJ.GET_STRING(''name_en''),
					P_NATIONALITY_NAME_EN => L_BODY_OBJ.GET_STRING(''nationality_name_en''),
...
'DECLARE
    V_RESPONSE      CLOB;
BEGIN
    SETUP_COUNTRY_TAPI.DELETE_COUNTRY(
        P_COUNTRY_ID          => :p_id
    );
END;'
      );
...
END;'
  ...
  ORDS.DEFINE_HANDLER(
      ...
      p_source         => 
'DECLARE
	V_CURRENCY_ID                   SETUP_CURRENCY.CURRENCY_ID%TYPE;
    V_TOKEN                          VARCHAR2(2000);
    L_BODY_OBJ                       JSON_OBJECT_T;
    P_VALID_RESULT                   JSON_OBJECT_T;
BEGIN
    ...
    L_BODY_OBJ := JSON_OBJECT_T(:body);
    -- step 2: call the procedure to validate inputs
    DATA_VALIDATION.CURRENCY_VALIDATE(
        P_DATA => L_BODY_OBJ,
        P_LANG => 2,
        P_RESULT => P_VALID_RESULT
    );
    IF P_VALID_RESULT.GET_STRING(''status'') = ''failed'' THEN
        ...
        return;
    END IF;
	V_CURRENCY_ID := L_BODY_OBJ.GET_STRING(''currency_id'');
    IF NVL(V_CURRENCY_ID,0) = 0 THEN
				SETUP_CURRENCY_TAPI.INSERT_CURRENCY (
					P_CURRENCY_ID => V_CURRENCY_ID,
					P_CREATED_BY  => L_BODY_OBJ.GET_STRING(''created_by''),
					P_NAME_EN     => L_BODY_OBJ.GET_STRING(''name_en''),
					P_CREATED_AT  => TO_DATE(L_BODY_OBJ.GET_STRING(''created_at''),''DD-MM-YYYY''),
					P_CODE_EN     => L_BODY_OBJ.GET_STRING(''code_en''),
					P_NAME_AR     => L_BODY_OBJ.GET_STRING(''name_ar''),
					P_CODE_AR     => L_BODY_OBJ.GET_STRING(''code_ar''),
					P_COMPANY_ID  => L_BODY_OBJ.GET_STRING(''company_id'')
				);
    ELSE
        SETUP_CURRENCY_TAPI.UPDATE_CURRENCY (
					P_CURRENCY_ID => L_BODY_OBJ.GET_STRING(''currency_id''),
					P_MODIFIED_AT  => TO_DATE(L_BODY_OBJ.GET_STRING(''modified_at''),''DD-MM-YYYY''),
					P_NAME_EN     => L_BODY_OBJ.GET_STRING(''name_en''),
...
'DECLARE
    V_RESPONSE      CLOB;
BEGIN
    SETUP_CURRENCY_TAPI.DELETE_CURRENCY(
        P_CURRENCY_ID          => :p_id
    );
END;'
      );
...
END;'
  ...
  ORDS.DEFINE_HANDLER(
      ...
      p_source         => 
'DECLARE
	V_PRODUCT_ID                   SALES_PRODUCT.PRODUCT_ID%TYPE;
    V_TOKEN                          VARCHAR2(2000);
    L_BODY_OBJ                       JSON_OBJECT_T;
    P_VALID_RESULT                   JSON_OBJECT_T;
BEGIN
    ...
    L_BODY_OBJ := JSON_OBJECT_T(:body);
    -- step 2: call the procedure to validate inputs
    DATA_VALIDATION.PRODUCT_VALIDATE(
        P_DATA => L_BODY_OBJ,
        P_LANG => 2,
        P_RESULT => P_VALID_RESULT
    );
    IF P_VALID_RESULT.GET_STRING(''status'') = ''failed'' THEN
        ...
        return;
    END IF;
	V_PRODUCT_ID := L_BODY_OBJ.GET_STRING(''product_id'');
    IF NVL(V_PRODUCT_ID,0) = 0 THEN
				SALES_PRODUCT_TAPI.INSERT_PRODUCT (
					P_PRODUCT_ID       => V_PRODUCT_ID,
					P_PRODUCT_NAME_AR  => L_BODY_OBJ.GET_STRING(''product_name_ar''),
					P_TAX_VALUE        => L_BODY_OBJ.GET_STRING(''tax_value''),
					P_SELL_PRICE       => L_BODY_OBJ.GET_STRING(''sell_price''),
					P_BARCODE          => L_BODY_OBJ.GET_STRING(''barcode''),
					P_PRODUCT_GROUP_ID => L_BODY_OBJ.GET_STRING(''product_group_id''),
					P_PROVIDER_ID      => L_BODY_OBJ.GET_STRING(''provider_id''),
					P_DEFAULT_UNIT_ID  => L_BODY_OBJ.GET_STRING(''default_unit_id''),
					P_TYPE_ID          => L_BODY_OBJ.GET_STRING(''type_id''),
					P_PRODUCT_STATUS   => L_BODY_OBJ.GET_STRING(''product_status''),
					P_PRODUCT_NO       => L_BODY_OBJ.GET_STRING(''product_no''),
					P_PRODUCT_NAME_EN  => L_BODY_OBJ.GET_STRING(''product_name_en''),
					P_PURCHASE_PRICE   => L_BODY_OBJ.GET_STRING(''purchase_price''),
					P_TAX_GROUP_ID     => L_BODY_OBJ.GET_STRING(''tax_group_id''),
					P_COMPANY_ID       => L_BODY_OBJ.GET_STRING(''company_id'')
				);
    ELSE
         SALES_PRODUCT_TAPI.UPDATE_PRODUCT (
					P_PRODUCT_ID       => L_BODY_OBJ.GET_STRING(''product_id''),
					P_PRODUCT_NAME_AR  => L_BODY_OBJ.GET_STRING(''product_name_ar''),
					P_TAX_VALUE        => L_BODY_OBJ.GET_STRING(''tax_value''),
					P_SELL_PRICE       => L_BODY_OBJ.GET_STRING(''sell_price''),
					P_BARCODE          => L_BODY_OBJ.GET_STRING(''barcode''),
...
'DECLARE
    V_RESPONSE      CLOB;
BEGIN
    SALES_PRODUCT_TAPI.DELETE_PRODUCT(
        P_PRODUCT_ID          => :p_id
    );
END;'
      );
...
'DECLARE
    ...
    V_RESPONSE      CLOB;
BEGIN
    V_RESPONSE := GET_PRODUCT_BARCODES(
        P_FIRST       => :p_first,
        P_LAST        => :p_last,
        P_NEXT        => :p_next,
        P_PREV        => :p_prev,
...
END;'
  ...
  ORDS.DEFINE_HANDLER(
      ...
      p_source         => 
'DECLARE
	V_PRODUCT_BARCODE_ID                   SALES_PRODUCT_BARCODES.PRODUCT_BARCODE_ID%TYPE;
    V_TOKEN                          VARCHAR2(2000);
    L_BODY_OBJ                       JSON_OBJECT_T;
    P_VALID_RESULT                   JSON_OBJECT_T;
BEGIN
    ...
    L_BODY_OBJ := JSON_OBJECT_T(:body);
    -- step 2: call the procedure to validate inputs
    DATA_VALIDATION.PRODUCT_BARCODE_VALIDATE(
        P_DATA => L_BODY_OBJ,
        P_LANG => 2,
        P_RESULT => P_VALID_RESULT
    );
    IF P_VALID_RESULT.GET_STRING(''status'') = ''failed'' THEN
        ...
        return;
    END IF;
	V_PRODUCT_BARCODE_ID := L_BODY_OBJ.GET_STRING(''product_barcode_id'');
    IF NVL(V_PRODUCT_BARCODE_ID,0) = 0 THEN

				SALES_PRODUCT_BARCODES_TAPI.INSERT_PRODUCT_BARCODES (
					P_PRODUCT_BARCODE_ID  => V_PRODUCT_BARCODE_ID,
					P_BARCODE             => L_BODY_OBJ.GET_STRING(''barcode''),
					P_PRODUCT_ID          => L_BODY_OBJ.GET_STRING(''product_id''),
					P_COMPANY_ID          => L_BODY_OBJ.GET_STRING(''company_id'')
				);
    ELSE
         SALES_PRODUCT_BARCODES_TAPI.UPDATE_PRODUCT_BARCODES (
					P_PRODUCT_BARCODE_ID  => L_BODY_OBJ.GET_STRING(''product_barcode_id''),
					P_BARCODE             => L_BODY_OBJ.GET_STRING(''barcode''),
					P_PRODUCT_ID          => L_BODY_OBJ.GET_STRING(''product_id''),
					P_COMPANY_ID          => L_BODY_OBJ.GET_STRING(''company_id'')
				);
...
'DECLARE
    V_RESPONSE      CLOB;
BEGIN
    SALES_PRODUCT_BARCODES_TAPI.DELETE_PRODUCT_BARCODES(
        P_PRODUCT_BARCODE_ID          => :p_id
    );
END;'
      );
  ORDS.DEFINE_TEMPLATE(
      p_module_name    => 'setup',
      p_pattern        => 'product_barcodes',
      p_priority       => 0,
      p_etag_type      => 'HASH',
      p_etag_query     => NULL,
      p_comments       => NULL);
...
END;'
  ...
  ORDS.DEFINE_HANDLER(
      ...
      p_source         => 
'DECLARE
	V_PRODUCT_UNIT_ID                   SALES_PRODUCT_UNIT.PRODUCT_UNIT_ID%TYPE;
    V_TOKEN                          VARCHAR2(2000);
    L_BODY_OBJ                       JSON_OBJECT_T;
    P_VALID_RESULT                   JSON_OBJECT_T;
BEGIN
    ...
    L_BODY_OBJ := JSON_OBJECT_T(:body);
    -- step 2: call the procedure to validate inputs
    DATA_VALIDATION.PRODUCT_UNIT_VALIDATE(
        P_DATA => L_BODY_OBJ,
        P_LANG => 2,
        P_RESULT => P_VALID_RESULT
    );
    IF P_VALID_RESULT.GET_STRING(''status'') = ''failed'' THEN
        ...
        return;
    END IF;
	V_PRODUCT_UNIT_ID := L_BODY_OBJ.GET_STRING(''product_unit_id'');
    IF NVL(V_PRODUCT_UNIT_ID,0) = 0 THEN

				SALES_PRODUCT_UNIT_TAPI.INSERT_PRODUCT_UNIT (
					P_PRODUCT_UNIT_ID => V_PRODUCT_UNIT_ID,
					P_UNIT_ID         => L_BODY_OBJ.GET_STRING(''unit_id''),
					P_UNIT_VALUE      => L_BODY_OBJ.GET_STRING(''unit_value''),
					P_SELL_PRICE      => L_BODY_OBJ.GET_STRING(''sell_price''),
					P_PURCHASE_PRICE  => L_BODY_OBJ.GET_STRING(''purchase_price''),
					P_BARCODE         => L_BODY_OBJ.GET_STRING(''barcode''),
					P_PRODUCT_ID      => L_BODY_OBJ.GET_STRING(''product_id''),
    ...
    ELSE
         SALES_PRODUCT_UNIT_TAPI.UPDATE_PRODUCT_UNIT (
					P_PRODUCT_UNIT_ID => L_BODY_OBJ.GET_STRING(''product_unit_id''),
					P_UNIT_ID         => L_BODY_OBJ.GET_STRING(''unit_id''),
					P_UNIT_VALUE      => L_BODY_OBJ.GET_STRING(''unit_value''),
					P_SELL_PRICE      => L_BODY_OBJ.GET_STRING(''sell_price''),
					P_PURCHASE_PRICE  => L_BODY_OBJ.GET_STRING(''purchase_price''),
...
'DECLARE
    V_RESPONSE      CLOB;
BEGIN
    SALES_PRODUCT_UNIT_TAPI.DELETE_PRODUCT_UNIT(
        P_PRODUCT_UNIT_ID          => :p_id
    );
END;'
      );
  ORDS.DEFINE_TEMPLATE(
      p_module_name    => 'setup',
      p_pattern        => 'product_units',
      p_priority       => 0,
      p_etag_type      => 'HASH',
      p_etag_query     => NULL,
      p_comments       => NULL);
...
END;'
  ...
  ORDS.DEFINE_HANDLER(
      ...
      p_source         => 
'DECLARE
	V_REGION_ID                   SETUP_REGION.REGION_ID%TYPE;
    V_TOKEN                          VARCHAR2(2000);
    L_BODY_OBJ                       JSON_OBJECT_T;
    P_VALID_RESULT                   JSON_OBJECT_T;
BEGIN
    ...
    L_BODY_OBJ := JSON_OBJECT_T(:body);
    -- step 2: call the procedure to validate inputs
    DATA_VALIDATION.REGION_VALIDATE(
        P_DATA => L_BODY_OBJ,
        P_LANG => 2,
        P_RESULT => P_VALID_RESULT
    );
    IF P_VALID_RESULT.GET_STRING(''status'') = ''failed'' THEN
        ...
        return;
    END IF;
	V_REGION_ID := L_BODY_OBJ.GET_STRING(''region_id'');
    IF NVL(V_REGION_ID,0) = 0 THEN

				SETUP_REGION_TAPI.INSERT_REGION (
					P_REGION_ID      => V_REGION_ID,
					P_COUNTRY_ID     => L_BODY_OBJ.GET_STRING(''country_id''),
					P_STATUS         => L_BODY_OBJ.GET_STRING(''status''),
					P_REGION_NAME_AR => L_BODY_OBJ.GET_STRING(''region_name_ar''),
					P_CITY_ID        => L_BODY_OBJ.GET_STRING(''city_id''),
					P_REGION_NAME_EN => L_BODY_OBJ.GET_STRING(''region_name_en''),
					P_COMPANY_ID     => L_BODY_OBJ.GET_STRING(''company_id'')
				);
    ELSE
         SETUP_REGION_TAPI.UPDATE_REGION (
					P_REGION_ID      => L_BODY_OBJ.GET_STRING(''region_id''),
					P_COUNTRY_ID     => L_BODY_OBJ.GET_STRING(''country_id''),
					P_STATUS         => L_BODY_OBJ.GET_STRING(''status''),
					P_REGION_NAME_AR => L_BODY_OBJ.GET_STRING(''region_name_ar''),
...
'DECLARE
    V_RESPONSE      CLOB;
BEGIN
    SETUP_REGION_TAPI.DELETE_REGION(
        P_REGION_ID          => :p_id
    );
END;'
      );
...
END;'
  ...
  ORDS.DEFINE_HANDLER(
      ...
      p_source         => 
'DECLARE
	V_UNIT_ID                   SETUP_UNIT.UNIT_ID%TYPE;
    V_TOKEN                          VARCHAR2(2000);
    L_BODY_OBJ                       JSON_OBJECT_T;
    P_VALID_RESULT                   JSON_OBJECT_T;
BEGIN
    ...
    IF P_VALID_RESULT.GET_STRING(''status'') = ''failed'' THEN
        ...
        return;
    END IF;
	V_UNIT_ID := L_BODY_OBJ.GET_STRING(''unit_id'');
    IF NVL(V_UNIT_ID,0) = 0 THEN
				SETUP_UNIT_TAPIINSERT_UNIT (
					P_UNIT_ID      => V_UNIT_ID,
					P_UNIT_NAME_AR => L_BODY_OBJ.GET_STRING(''unit_name_ar''),
					P_UNIT_NAME_EN => L_BODY_OBJ.GET_STRING(''unit_name_en''),
					P_COMPANY_ID   => L_BODY_OBJ.GET_STRING(''company_id'')
				);
    ELSE
         SETUP_UNIT_TAPI.UPDATE_UNIT (
					P_UNIT_ID      => L_BODY_OBJ.GET_STRING(''unit_id''),
					P_UNIT_NAME_AR => L_BODY_OBJ.GET_STRING(''unit_name_ar''),
					P_UNIT_NAME_EN => L_BODY_OBJ.GET_STRING(''unit_name_en''),
					P_COMPANY_ID   => L_BODY_OBJ.GET_STRING(''company_id'')
